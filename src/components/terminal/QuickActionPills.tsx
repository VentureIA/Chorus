...
export function QuickActionPills({ onAction, onManageClick }: QuickActionPillsProps) {
  ...
  return (
    <div className="no-select flex shrink-0 items-center gap-1 border-t border-border bg-muted px-2 py-1">
      {sortedActions.map((a) => (
        <button
          type="button"
          key={a.id}
          ...
          className={`inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[10px] text-muted-foreground transition-colors hover:bg-card hover:text-foreground${!onAction ? " opacity-50 cursor-not-allowed" : ""}`}
        >
          <DynamicIcon
            name={a.icon}
            size={9}
      ...
      <button
        ...
        className={`inline-flex items-center justify-center rounded p-0.5 text-muted-foreground transition-colors hover:bg-card hover:text-foreground${!onManageClick ? " opacity-50 cursor-not-allowed" : ""}`}
      >
        <Plus size={11} />
      </button>
    </div>
