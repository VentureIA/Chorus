      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-maestro-border px-4 py-3">
          <div className="flex items-center gap-2">
            <Keyboard size={16} className="text-maestro-accent" />
        ...
        <div className="max-h-[70vh] overflow-y-auto p-4">
          <div className="space-y-6">
            {categoryOrder.map((category) => {
              ...
              return (
                <div key={category}>
                  <h3 className="mb-3 text-xs font-semibold uppercase tracking-wide text-maestro-muted">
                    {info.label}
                  </h3>
...
function ShortcutRow({
  ...
  if (isEditing) {
    return (
      <div className="rounded-lg border border-maestro-accent/50 bg-maestro-card p-3">
        <div className="mb-2 flex items-center justify-between">
          <span className="text-sm text-maestro-text">{shortcut.label}</span>
        </div>
        <ShortcutRecorder
          value={shortcut.keys}
          onChange={onSave}
  ...
  return (
    <div
      className="group flex items-center justify-between rounded-lg border border-transparent px-3 py-2 hover:border-maestro-border hover:bg-maestro-card/50"
    >
      <div className="flex flex-col">
        <span className="text-sm text-maestro-text">{shortcut.label}</span>
        <span className="text-xs text-maestro-muted">{shortcut.description}</span>
      </div>
      <div className="flex items-center gap-2">
        {isModified && (
          <button
            onClick={onReset}
            className="rounded p-1 text-maestro-muted opacity-0 hover:bg-maestro-border/40 group-hover:opacity-100"
        ...
        <button
          onClick={onEdit}
          className="flex items-center gap-1.5 rounded border border-maestro-border bg-maestro-surface px-2 py-1 font-mono text-xs text-maestro-text hover:border-maestro-accent"
        >
          <span>{formatKeyCombo(shortcut.keys)}</span>
          <Edit2 size={10} className="text-maestro-muted opacity-0 group-hover:opacity-100" />
        </button>
      </div>
    </div>
  );
